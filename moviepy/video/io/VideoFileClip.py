...
class VideoFileClip(VideoClip):
    ...
    Note that this creates subprocesses and locks files. If you construct one of these instances,
    you must call close() afterwards, or the subresources will not be
    cleaned up until the process ends.

    If copies are made, and close() is called on one,
    it may cause methods on the other copies to fail.
    ...
    def __init__(
        ...
        if has_mask:
            ...
            self.mask = VideoClip(
                ismask=True,
                make_frame=lambda t: self.reader.get_frame(t)[:, :, 3] / 255.0,
